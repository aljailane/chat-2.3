...
*/
echo "<title>$config->title &raquo; Moderat0r Menu</title>"; if(isset($_GET["action"])) { $action=$_GET["action"]; }
else { $action=" "; }
if($action=="mjatdmngdgmpwelete")
...
elseif($action=="pwmdmamjmm") { if(isset($_POST["submit"])) { $message=cleanvalues($_POST["message"]); $reciever=cleanvalues($_POST["reciever"]);
...
{ echo"<ul><div class='center'>Please Be Wise While Taking Action On Users !</div></ul><br>";
$msg=$_GET["msg"];
if(!empty($msg))
...
if($_GET["show"]=="admamod") { echo "<div class=\"b_head\"><b>Moderators List</b></div><br/><div class=\"topnav\"><a href='?'>Users List</a></div>";
$query=mysql_query("SELECT * FROM b_users WHERE level=1 LIMIT $offset, $max"); } else { echo "<div class=\"b_head\"><b>Users List</b></div>";
...
echo "<div class=\"user-info\"><ul><li>[$id] $st $username<br/>$b - <a href='../profile.php?uid=$id'>Info</a></li></ul></div>"; }
